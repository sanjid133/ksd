...
var ksdsResource = schema.GroupVersionResource{Group: "sho.k8s.io", Version: "", Resource: "ksds"}

var ksdsKind = schema.GroupVersionKind{Group: "sho.k8s.io", Version: "", Kind: "Ksd"}

// Get takes name of the ksd, and returns the corresponding ksd object, and an error if there is any.
func (c *FakeKsds) Get(name string, options v1.GetOptions) (result *sho.Ksd, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(ksdsResource, c.ns, name), &sho.Ksd{})

	if obj == nil {
		return nil, err
	}
	return obj.(*sho.Ksd), err
}

// List takes label and field selectors, and returns the list of Ksds that match those selectors.
func (c *FakeKsds) List(opts v1.ListOptions) (result *sho.KsdList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(ksdsResource, ksdsKind, c.ns, opts), &sho.KsdList{})
	...
	list := &sho.KsdList{}
	for _, item := range obj.(*sho.KsdList).Items {
...
func (c *FakeKsds) Create(ksd *sho.Ksd) (result *sho.Ksd, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(ksdsResource, c.ns, ksd), &sho.Ksd{})
...
func (c *FakeKsds) Update(ksd *sho.Ksd) (result *sho.Ksd, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(ksdsResource, c.ns, ksd), &sho.Ksd{})
...
func (c *FakeKsds) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(ksdsResource, c.ns, name), &sho.Ksd{})
...
func (c *FakeKsds) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	...
	_, err := c.Fake.Invokes(action, &sho.KsdList{})
	return err
}

// Patch applies the patch and returns the patched ksd.
func (c *FakeKsds) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *sho.Ksd, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(ksdsResource, c.ns, name, data, subresources...), &sho.Ksd{})
